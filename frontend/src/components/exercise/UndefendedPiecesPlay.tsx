...
export type UndefendedMode = "practice" | "speed";

// Dedicated Practice + Speed loop for Undefended Pieces. Renders and
...
// The undefended squares stay server-authoritative and are never revealed
// before submission.
//
...
export function UndefendedPiecesPlay({ mode }: { mode: UndefendedMode }) {
...
function PracticeLoop() {
  ...
  async function refill(exclude: number[]) {
    ...
    try {
      next = await api.nextUndefendedPracticePuzzle({ exclude_ids: exclude });
    } catch {
      return;
    }
  ...
  async function loadInitial() {
    ...
    try {
      first = await api.nextUndefendedPracticePuzzle({ exclude_ids: [] });
    } catch {
      first = null;
    }
    if (!alive(gen)) return;
  ...
  function advance() {
    ...
    if (next && (!current || next.id !== current.id)) {
      ...
      setPhase("loading");
      api
        .nextUndefendedPracticePuzzle({ exclude_ids: current ? [current.id] : [] })
        .then((puzzle) => {
          if (!alive(gen)) return;
          resetFor(puzzle);
          refill([puzzle.id]);
  ...
  if ((phase === "loading" || phase === "error") && !current) {
    return (
      <GameShell title={t("undefended.title")} modeKey="play.practice">
  ...
  const action =
    phase === "feedback" ? (
      ...
      <SubmitBar
        ...
        emptyHint={t("undefended.emptyAllowed")}
  ...
  return (
    <GameShell title={t("undefended.title")} modeKey="play.practice" stat={stat}>
...
function SpeedLoop() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (sid && sessionAliveRef.current) api.finishUndefendedSpeedSession(sid).catch(() => null);
    };
  }, []);
  ...
  async function dieToReport(sessionId: string, gen: number) {
    ...
    try {
      const data = await api.getUndefendedSpeedReport(sessionId);
  ...
  async function finishToReport(sessionId: string, gen: number) {
    ...
    cancelTransition();
    try {
      await api.finishUndefendedSpeedSession(sessionId).catch(() => null);
      const data = await api.getUndefendedSpeedReport(sessionId);
      if (!alive(gen) || sessionIdRef.current !== sessionId) return;
      setReport(data);
      setSession(data.session);
  ...
  async function boot() {
    ...
    setPhase("preparing");
    try {
      if (prev) await api.finishUndefendedSpeedSession(prev).catch(() => null);
      const created = await api.startUndefendedSpeedSession();
      if (!alive(gen)) return;
      ...
      while (acc.length < MIN_BUFFER && guard < 6) {
        guard += 1;
        const batch = await api.prepareUndefendedSpeedPuzzles(created.session_id, { count: MIN_BUFFER });
      ...
      const started = await api.startUndefendedSpeedClock(created.session_id);
      if (!alive(gen) || sessionIdRef.current !== created.session_id) return;
      setSession({
        ...started,
        attempted: 0,
  ...
  async function refillIfNeeded() {
    ...
    try {
      const batch = await api.prepareUndefendedSpeedPuzzles(sid, { count: REFILL_COUNT });
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const res = await api.submitUndefendedSpeedAnswer(sid, {
        puzzle_id: puzzle.id,
  ...
  async function advance() {
    ...
    setError(null);
    try {
      const puzzle = await api.nextUndefendedSpeedPuzzle(sid);
      const gen = genRef.current;
      if (!alive(gen) || sessionIdRef.current !== sid) return;
      resetFor(puzzle);
      refillIfNeeded();
  ...
  if (phase === "preparing") {
    ...
    return (
      <GameShell title={t("undefended.title")} modeKey="play.speed">
        <div className="grid min-h-0 flex-1 place-items-center px-4">
          <Card>

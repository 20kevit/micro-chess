...
export type CaptureMode = "practice" | "speed";

// Dedicated Practice + Speed loop for Captures. Renders and
...
// intermediate mode-selection screen. The hunter (white attacker) square
// comes from the puzzle's position_json (part of the task); the capturable
// black-piece squares stay server-authoritative and are never revealed
// before submission. Whether a black piece is defended never matters here
// (that is Exercise 4); only "can the hunter capture it" counts.
//
...
export function CapturesPlay({ mode }: { mode: CaptureMode }) {
  return mode === "practice" ? <PracticeLoop /> : <SpeedLoop />;
}

/** Task square highlighted on the board (the white hunter piece). */
function targetOf(puzzle: Puzzle): string | null {
  const from = (puzzle.position_json as Record<string, unknown>).from;
...
function PracticeLoop() {
  ...
  async function refill(exclude: number[]) {
    ...
    try {
      next = await api.nextCapturePracticePuzzle({ exclude_ids: exclude });
    } catch {
      return;
    }
  ...
  async function loadInitial() {
    ...
    try {
      first = await api.nextCapturePracticePuzzle({ exclude_ids: [] });
    } catch {
      first = null;
    }
    if (!alive(gen)) return;
  ...
  function advance() {
    ...
    if (next && (!current || next.id !== current.id)) {
      ...
      setPhase("loading");
      api
        .nextCapturePracticePuzzle({ exclude_ids: current ? [current.id] : [] })
        .then((puzzle) => {
          if (!alive(gen)) return;
          resetFor(puzzle);
          refill([puzzle.id]);
  ...
  if ((phase === "loading" || phase === "error") && !current) {
    return (
      <GameShell title={t("captures.title")} modeKey="play.practice">
  ...
  const action =
    phase === "feedback" ? (
      ...
      <SubmitBar
        ...
        emptyHint={t("captures.emptyAllowed")}
  ...
  return (
    <GameShell title={t("captures.title")} modeKey="play.practice" stat={stat}>
...
function SpeedLoop() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (sid && sessionAliveRef.current) api.finishCaptureSpeedSession(sid).catch(() => null);
    };
  }, []);
  ...
  async function dieToReport(sessionId: string, gen: number) {
    ...
    try {
      const data = await api.getCaptureSpeedReport(sessionId);
  ...
  async function finishToReport(sessionId: string, gen: number) {
    ...
    cancelTransition();
    try {
      await api.finishCaptureSpeedSession(sessionId).catch(() => null);
      const data = await api.getCaptureSpeedReport(sessionId);
      if (!alive(gen) || sessionIdRef.current !== sessionId) return;
      setReport(data);
      setSession(data.session);
  ...
  async function boot() {
    ...
    setPhase("preparing");
    try {
      if (prev) await api.finishCaptureSpeedSession(prev).catch(() => null);
      const created = await api.startCaptureSpeedSession();
      if (!alive(gen)) return;
      ...
      while (acc.length < MIN_BUFFER && guard < 6) {
        guard += 1;
        const batch = await api.prepareCaptureSpeedPuzzles(created.session_id, { count: MIN_BUFFER });
      ...
      const started = await api.startCaptureSpeedClock(created.session_id);
      if (!alive(gen) || sessionIdRef.current !== created.session_id) return;
      setSession({
        ...started,
        attempted: 0,
  ...
  async function refillIfNeeded() {
    ...
    try {
      const batch = await api.prepareCaptureSpeedPuzzles(sid, { count: REFILL_COUNT });
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const res = await api.submitCaptureSpeedAnswer(sid, {
        puzzle_id: puzzle.id,
  ...
  async function advance() {
    ...
    setError(null);
    try {
      const puzzle = await api.nextCaptureSpeedPuzzle(sid);
      const gen = genRef.current;
      if (!alive(gen) || sessionIdRef.current !== sid) return;
      resetFor(puzzle);
      refillIfNeeded();
  ...
  if (phase === "preparing") {
    ...
    return (
      <GameShell title={t("captures.title")} modeKey="play.speed">
        <div className="grid min-h-0 flex-1 place-items-center px-4">
          <Card>

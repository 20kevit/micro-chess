...
export type LegalMode = "practice" | "speed";

// Dedicated Practice + Speed loop for Legal Destinations. Renders and
...
// intermediate mode-selection screen. The target piece square comes from
// the puzzle's position_json (part of the task); destinations stay
// server-authoritative and are never revealed before submission.
//
...
export function LegalDestinationsPlay({ mode }: { mode: LegalMode }) {
  return mode === "practice" ? <PracticeLoop /> : <SpeedLoop />;
}

/** Task square highlighted on the board (the target piece). */
...
function PracticeLoop() {
  ...
  async function refill(exclude: number[]) {
    ...
    try {
      next = await api.nextLegalPracticePuzzle({ exclude_ids: exclude });
    } catch {
      return;
    }
  ...
  async function loadInitial() {
    ...
    try {
      first = await api.nextLegalPracticePuzzle({ exclude_ids: [] });
    } catch {
      first = null;
    }
    if (!alive(gen)) return;
  ...
  if ((phase === "loading" || phase === "error") && !current) {
    return (
      <GameShell title={t("legal.title")} modeKey="play.practice">
  ...
  const action =
    phase === "feedback" ? (
      ...
      <SubmitBar
        ...
        emptyHint={t("legal.emptyAllowed")}
  ...
  return (
    <GameShell title={t("legal.title")} modeKey="play.practice" stat={stat}>
...
function SpeedLoop() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (sid && sessionAliveRef.current) api.finishLegalSpeedSession(sid).catch(() => null);
    };
  }, []);
  ...
  async function dieToReport(sessionId: string, gen: number) {
    ...
    try {
      const data = await api.getLegalSpeedReport(sessionId);
  ...
  async function finishToReport(sessionId: string, gen: number) {
    ...
    cancelTransition();
    try {
      await api.finishLegalSpeedSession(sessionId).catch(() => null);
      const data = await api.getLegalSpeedReport(sessionId);
      if (!alive(gen) || sessionIdRef.current !== sessionId) return;
      setReport(data);
      setSession(data.session);
  ...
  async function boot() {
    ...
    setPhase("preparing");
    try {
      if (prev) await api.finishLegalSpeedSession(prev).catch(() => null);
      const created = await api.startLegalSpeedSession();
      if (!alive(gen)) return;
      ...
      while (acc.length < MIN_BUFFER && guard < 6) {
        guard += 1;
        const batch = await api.prepareLegalSpeedPuzzles(created.session_id, { count: MIN_BUFFER });
      ...
      const started = await api.startLegalSpeedClock(created.session_id);
      if (!alive(gen) || sessionIdRef.current !== created.session_id) return;
      setSession({
        ...started,
        attempted: 0,
  ...
  async function refillIfNeeded() {
    ...
    try {
      const batch = await api.prepareLegalSpeedPuzzles(sid, { count: REFILL_COUNT });
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const res = await api.submitLegalSpeedAnswer(sid, {
        puzzle_id: puzzle.id,
  ...
  async function advance() {
    ...
    setError(null);
    try {
      const puzzle = await api.nextLegalSpeedPuzzle(sid);
      const gen = genRef.current;
      if (!alive(gen) || sessionIdRef.current !== sid) return;
      resetFor(puzzle);
      refillIfNeeded();
  ...
  if (phase === "preparing") {
    ...
    return (
      <GameShell title={t("legal.title")} modeKey="play.speed">
        <div className="grid min-h-0 flex-1 place-items-center px-4">
          <Card>

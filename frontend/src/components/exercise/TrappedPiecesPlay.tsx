...
export type TrappedMode = "practice" | "speed";

// Dedicated Practice + Speed loop for Trapped Pieces. Renders and
...
// intermediate mode-selection screen. The whole board is the question, so
// nothing is highlighted up front and only the user's selections are shown.
// The trapped squares stay server-authoritative and are never revealed
// before submission.
//
// Practice vs Speed (deliberate asymmetry):
// - Practice: select any number of pieces, change freely, press
//   «بررسی جواب». Positions may hold 1-3 trapped pieces.
// - Speed: every position holds EXACTLY ONE trapped piece, so tapping a
//   piece submits immediately with no confirm step (+5 correct / -4 wrong:
//   -2 for the wrong tap plus -2 for the missed trapped piece).
// Scoring (+5 correct / -2 missed / -2 wrong per square) is
// backend-authoritative in both modes.
//
...
export function TrappedPiecesPlay({ mode }: { mode: TrappedMode }) {
  return mode === "practice" ? <PracticeLoop /> : <SpeedLoop />;
}

/** No pre-highlight: the whole board is the question. */
function targetOf(_puzzle: Puzzle): string | null {
  return null;
}
...
function PracticeLoop() {
  ...
  async function refill(exclude: number[]) {
    ...
    try {
      next = await api.nextTrappedPracticePuzzle({ exclude_ids: exclude });
    } catch {
      return;
    }
  ...
  async function loadInitial() {
    ...
    try {
      first = await api.nextTrappedPracticePuzzle({ exclude_ids: [] });
    } catch {
      first = null;
    }
    if (!alive(gen)) return;
  ...
  function advance() {
    ...
    if (next && (!current || next.id !== current.id)) {
      ...
      setPhase("loading");
      api
        .nextTrappedPracticePuzzle({ exclude_ids: current ? [current.id] : [] })
        .then((puzzle) => {
          if (!alive(gen)) return;
          resetFor(puzzle);
          refill([puzzle.id]);
  ...
  if ((phase === "loading" || phase === "error") && !current) {
    return (
      <GameShell title={t("trapped.title")} modeKey="play.practice">
  ...
  const board = (
    <BoardZone
      ...
      disabled={!answering}
      onToggle={toggle}
      targetSquare={targetOf(puzzle)}
    />
  );

  return (
    <GameShell title={t("trapped.title")} modeKey="play.practice" stat={stat}>
...
function SpeedLoop() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (sid && sessionAliveRef.current) api.finishTrappedSpeedSession(sid).catch(() => null);
    };
  }, []);
  ...
  async function dieToReport(sessionId: string, gen: number) {
    ...
    try {
      const data = await api.getTrappedSpeedReport(sessionId);
  ...
  async function finishToReport(sessionId: string, gen: number) {
    ...
    cancelTransition();
    try {
      await api.finishTrappedSpeedSession(sessionId).catch(() => null);
      const data = await api.getTrappedSpeedReport(sessionId);
      if (!alive(gen) || sessionIdRef.current !== sessionId) return;
      setReport(data);
      setSession(data.session);
  ...
  async function boot() {
    ...
    setPhase("preparing");
    try {
      if (prev) await api.finishTrappedSpeedSession(prev).catch(() => null);
      const created = await api.startTrappedSpeedSession();
      if (!alive(gen)) return;
      ...
      while (acc.length < MIN_BUFFER && guard < 6) {
        guard += 1;
        const batch = await api.prepareTrappedSpeedPuzzles(created.session_id, { count: MIN_BUFFER });
      ...
      const started = await api.startTrappedSpeedClock(created.session_id);
      if (!alive(gen) || sessionIdRef.current !== created.session_id) return;
      setSession({
        ...started,
        attempted: 0,
  ...
  async function refillIfNeeded() {
    ...
    try {
      const batch = await api.prepareTrappedSpeedPuzzles(sid, { count: REFILL_COUNT });
  ...
  function toggle(square: string) {
    ...
    // Speed rule: one tap IS the answer. Every Speed position holds
    // exactly one trapped piece, so the tapped square submits immediately
    // with no confirm step and no deselect.
    setSelected([square]);
    submit([square]);
  }

  async function submit(squares: string[]) {
    ...
    setError(null);
    try {
      const res = await api.submitTrappedSpeedAnswer(sid, {
        puzzle_id: puzzle.id,
        answer: { selected_squares: squares },
  ...
  async function advance() {
    ...
    setError(null);
    try {
      const puzzle = await api.nextTrappedSpeedPuzzle(sid);
      const gen = genRef.current;
      if (!alive(gen) || sessionIdRef.current !== sid) return;
      resetFor(puzzle);
      refillIfNeeded();
  ...
  if (phase === "preparing") {
    ...
    return (
      <GameShell title={t("trapped.title")} modeKey="play.speed">
        <div className="grid min-h-0 flex-1 place-items-center px-4">
          <Card>
  ...
  // Speed has no confirm button: tapping a piece submits immediately
  // (see toggle/submit above). The bar only states that rule.
  const action = (
    <div className="shrink-0 px-3 pb-3 pt-1" data-testid="submit-bar">
      <p className="mb-1 truncate text-center text-xs font-bold text-stone-600">{t("trapped.speedHint")}</p>
    </div>
  );

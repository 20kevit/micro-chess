...
export type GetOutOfCheckMode = "practice" | "speed";
...
// Dedicated Practice + Speed loop for Get Out of Check. Renders
...
//
// The answer is a SET of arrows (from->to escaping moves). Arrows are
...
export function GetOutOfCheckPlay({ mode }: { mode: GetOutOfCheckMode }) {
...
function ArrowList({
  ...
  return (
    <div className="shrink-0 px-3" data-testid="arrow-list">
      <p className="mb-1 text-center text-xs font-bold text-stone-600">{t("getoutcheck.arrows")}</p>
        ...
        {arrows.map((a) => {
          ...
          return (
            ...
            >
              ...
              {needsPromotion ? (
                <div className="flex shrink-0 gap-1" role="group" aria-label={t("getoutcheck.promotion")}>
              ...
              <button
                type="button"
                aria-label={`${t("getoutcheck.removeArrow")} ${a.from}→${a.to}`}
                disabled={disabled}
...
function PracticeLoop() {
  ...
  async function refill(exclude: number[]) {
    ...
    try {
      next = await api.nextGetOutOfCheckPracticePuzzle({ exclude_ids: exclude });
    } catch {
      return;
    }
  ...
  async function loadInitial() {
    ...
    try {
      first = await api.nextGetOutOfCheckPracticePuzzle({ exclude_ids: [] });
    } catch {
      first = null;
    }
    if (!alive(gen)) return;
  ...
  function advance() {
    ...
    if (next && (!current || next.id !== current.id)) {
      ...
      setPhase("loading");
      api
        .nextGetOutOfCheckPracticePuzzle({ exclude_ids: current ? [current.id] : [] })
        .then((puzzle) => {
          if (!alive(gen)) return;
          resetFor(puzzle);
          refill([puzzle.id]);
  ...
  if ((phase === "loading" || phase === "error") && !current) {
    return (
      <GameShell title={t("getoutcheck.title")} modeKey="play.practice">
  ...
  return (
    <GameShell title={t("getoutcheck.title")} modeKey="play.practice" stat={stat}>
...
function SpeedLoop() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (sid && sessionAliveRef.current) api.finishGetOutOfCheckSpeedSession(sid).catch(() => null);
    };
  }, []);
  ...
  async function dieToReport(sessionId: string, gen: number) {
    ...
    try {
      const data = await api.getGetOutOfCheckSpeedReport(sessionId);
  ...
  async function finishToReport(sessionId: string, gen: number) {
    ...
    cancelTransition();
    try {
      await api.finishGetOutOfCheckSpeedSession(sessionId).catch(() => null);
      const data = await api.getGetOutOfCheckSpeedReport(sessionId);
      if (!alive(gen) || sessionIdRef.current !== sessionId) return;
      setReport(data);
      setSession(data.session);
  ...
  async function boot() {
    ...
    setPhase("preparing");
    try {
      if (prev) await api.finishGetOutOfCheckSpeedSession(prev).catch(() => null);
      const created = await api.startGetOutOfCheckSpeedSession();
      if (!alive(gen)) return;
      ...
      while (acc.length < MIN_BUFFER && guard < 6) {
        guard += 1;
        const batch = await api.prepareGetOutOfCheckSpeedPuzzles(created.session_id, { count: MIN_BUFFER });
      ...
      const started = await api.startGetOutOfCheckSpeedClock(created.session_id);
      if (!alive(gen) || sessionIdRef.current !== created.session_id) return;
      setSession({
        ...started,
        attempted: 0,
  ...
  async function refillIfNeeded() {
    ...
    try {
      const batch = await api.prepareGetOutOfCheckSpeedPuzzles(sid, { count: REFILL_COUNT });
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const res = await api.submitGetOutOfCheckSpeedAnswer(sid, {
        puzzle_id: puzzle.id,
  ...
  async function advance() {
    ...
    setError(null);
    try {
      const puzzle = await api.nextGetOutOfCheckSpeedPuzzle(sid);
      const gen = genRef.current;
      if (!alive(gen) || sessionIdRef.current !== sid) return;
      resetFor(puzzle);
      refillIfNeeded();
  ...
  if (phase === "preparing") {
    ...
    return (
      <GameShell title={t("getoutcheck.title")} modeKey="play.speed">
        <div className="grid min-h-0 flex-1 place-items-center px-4">
          <Card>

...
export type PathfindingMode = "practice" | "speed";
...
// Dedicated Practice + Speed loop for Pathfinding (Exercise 6). Renders
// and transports answers only; legality, optimal counts, scoring, and
// the speed clock stay backend-authoritative. The ?mode= URL param picks
// the loop directly; there is no intermediate mode-selection screen.
//
// One white piece walks to the star on an empty board (no king, no
// enemies, no captures). Primary input is drag-and-drop; click-select
// then click-destination works too. The piece STAYS selected after every
// move (legal or illegal), so destinations can be clicked repeatedly.
// Reaching the star auto-submits the full path as one attempt; the
// server recomputes the optimal distance and the score from scratch.
export function PathfindingPlay({ mode }: { mode: PathfindingMode }) {
...
export function posOf(puzzle: Puzzle): { from: string; target: string; piece: string } {
...
function PracticeLoop() {
  ...
  async function refill(exclude: number[]) {
    ...
    try {
      next = await api.nextPathfindingPracticePuzzle({ exclude_ids: exclude });
    } catch {
      return;
    }
  ...
  async function loadInitial() {
    ...
    try {
      first = await api.nextPathfindingPracticePuzzle({ exclude_ids: [] });
    } catch {
      first = null;
    }
    if (!alive(gen)) return;
  ...
  function advance() {
    ...
    if (next && (!current || next.id !== current.id)) {
      ...
      setPhase("loading");
      api
        .nextPathfindingPracticePuzzle({ exclude_ids: current ? [current.id] : [] })
        .then((puzzle) => {
          if (!alive(gen)) return;
          resetFor(puzzle);
          refill([puzzle.id]);
  ...
  if ((phase === "loading" || phase === "error") && !current) {
    return (
      <GameShell title={t("exercises.pathfinding.title")} modeKey="play.practice">
  ...
  return (
    <GameShell title={t("exercises.pathfinding.title")} modeKey="play.practice" stat={stat}>
...
function SpeedLoop() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (sid && sessionAliveRef.current) api.finishPathfindingSpeedSession(sid).catch(() => null);
    };
  }, []);
  ...
  async function dieToReport(sessionId: string, gen: number) {
    ...
    try {
      const data = await api.getPathfindingSpeedReport(sessionId);
  ...
  async function finishToReport(sessionId: string, gen: number) {
    ...
    cancelTransition();
    try {
      await api.finishPathfindingSpeedSession(sessionId).catch(() => null);
      const data = await api.getPathfindingSpeedReport(sessionId);
      if (!alive(gen) || sessionIdRef.current !== sessionId) return;
      setReport(data);
      setSession(data.session);
  ...
  async function boot() {
    ...
    setPhase("preparing");
    try {
      if (prev) await api.finishPathfindingSpeedSession(prev).catch(() => null);
      const created = await api.startPathfindingSpeedSession();
      if (!alive(gen)) return;
      ...
      while (acc.length < MIN_BUFFER && guard < 6) {
        guard += 1;
        const batch = await api.preparePathfindingSpeedPuzzles(created.session_id, { count: MIN_BUFFER });
      ...
      const started = await api.startPathfindingSpeedClock(created.session_id);
      if (!alive(gen) || sessionIdRef.current !== created.session_id) return;
  ...
  async function refillIfNeeded() {
    ...
    try {
      const batch = await api.preparePathfindingSpeedPuzzles(sid, { count: REFILL_COUNT });
  ...
  async function submitFinal(finalPath: string[], illegalCount: number) {
    ...
    setError(null);
    try {
      const res = await api.submitPathfindingSpeedAnswer(sid, {
        puzzle_id: puzzle.id,
        answer: answerForPath(finalPath, illegalCount),
  ...
  async function advance() {
    ...
    setError(null);
    try {
      const puzzle = await api.nextPathfindingSpeedPuzzle(sid);
      const gen = genRef.current;
      if (!alive(gen) || sessionIdRef.current !== sid) return;
      resetFor(puzzle);
      refillIfNeeded();
  ...
  if (phase === "preparing") {
    return (
      <GameShell title={t("exercises.pathfinding.title")} modeKey="play.speed">
        <div className="grid min-h-0 flex-1 place-items-center px-4">
          <Card>

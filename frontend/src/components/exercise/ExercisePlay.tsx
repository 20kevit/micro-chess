...
//
// Viewport-fit by construction (global screen-fit rule): play renders in
// the fixed `GameShell` overlay with a measured board (`useGameFit`), so
// prompt + board + controls + feedback always fit without page scrolling,
// portrait or landscape. Future page-flow exercises inherit this by using
// this component instead of hand-rolling page layout.
export function ExercisePlay({ config }: { config: ExercisePlayConfig }) {
  ...
  const modeKey: FaKey = mode === "practice" ? "play.practice" : "play.rated";

  if (!started) {
    return (
      <GameShell title={t(config.titleKey)} modeKey={modeKey}>
        <div className="grid min-h-0 flex-1 place-items-center overflow-y-auto px-4 py-3">
          <Card className="w-full max-w-md">
            <p className="text-sm text-stone-500">{t(config.introKey)}</p>
            {config.exerciseModes ? (
              <>
                <p className="mb-1 mt-3 text-sm text-stone-500">
            ...
            <p className="mb-1 mt-3 text-sm text-stone-500">{t("play.mode")}</p>
...
function PlayLoop({
      ...
      <GameShell title={t(config.titleKey)} modeKey={modeKey}>
        <div className="grid min-h-0 flex-1 place-items-center overflow-y-auto px-4 py-3">
          <Card className="w-full max-w-md">
            ...
            <Button className="mt-3 w-full" onClick={load}>
              {t("common.retry")}
            </Button>
          </Card>
        </div>
  ...
  const stat = `${faNum(index + 1)} / ${faNum(shownPuzzles?.length ?? 0)}`;

  const promptRow = (
    <div className="flex min-w-0 shrink-0 items-center gap-2 px-3 pt-1" data-testid="question">
      <p className="min-w-0 flex-1 text-center text-base font-black leading-7 text-stone-900">
        {puzzle.prompt_fa}
      </p>
      {config.directPlay ? (
        ...
        <button
          type="button"
          className="min-h-[44px] shrink-0 rounded-full bg-violet-100 px-3 text-sm font-bold text-violet-700"
    ...
    <div className="shrink-0 px-3 pb-3">
      ...
      <div
        className={`mt-2 flex gap-2 ${
          orientation === "landscape" ? "sticky bottom-0 bg-[#f6f4ff] py-1" : ""
        }`}
      ...
      >
        ...
        <Card className="mt-3 max-h-44 overflow-y-auto">
      ...
      <Card>
        ...
        <div
          className={`mt-3 flex gap-2 ${
            orientation === "landscape" ? "sticky bottom-0 bg-white py-1" : ""
          }`}
        >
  ...
  return (
    <GameShell title={t(config.titleKey)} modeKey={modeKey} stat={stat}>
      <div ref={rootRef} className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        {orientation === "landscape" ? (
          <div className="flex min-h-0 min-w-0 flex-1">
            {board}
            {/* Wider than sibling asides (w-60): option/feedback stacks need
              the room, and the height-bound board loses nothing. */}
            <aside className="flex w-72 shrink-0 flex-col gap-1 overflow-y-auto py-1">
              {promptRow}
              {answerControls}
              {resultCard}
            </aside>

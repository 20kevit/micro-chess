...
// Blindfold task payload (visible). The FEN and the solution UCIs never
// travel to the client: only the Persian description, side to move, mode
// and opening context do.
function asTask(puzzle: Puzzle): {
  description: string;
  side: string;
  opening: string;
  trap: string;
  ...
  if (
    typeof raw.description_fa !== "string" ||
    typeof raw.side_to_move !== "string" ||
    typeof raw.opening_fa !== "string" ||
    typeof raw.trap_fa !== "string" ||
    typeof raw.theme_fa !== "string"
  ) {
    return null;
  }
  return {
    description: raw.description_fa,
    side: raw.side_to_move,
    opening: raw.opening_fa,
    trap: raw.trap_fa,
    themeFa: raw.theme_fa,
  };
}

function detailText(result: AttemptResponse, key: "correct_move" | "theme"): string | null {
  const raw = result.detail[key];
  return typeof raw === "string" && raw.length > 0 ? raw : null;
}

// Text-answer play loop for Opening Traps Blindfold. Same Entry → Play →
// Feedback → Next shape and primitives as the other play loops (attempts,
// timing, hints), but deliberately renders NO chessboard, NO piece images,
// NO FEN and NO solution data: the opening context plus the Persian
// position description is the whole interface. The backend parses the SAN
// against the real position and accepts every configured tactic.
export function OpeningTrapsPlay() {
  ...
  if (!started) {
    return (
      <div>
        <PageHeader title={t("exercises.opening-traps.title")} subtitle={t("openingTraps.intro")} />
        <Card>
  ...
  return <OpeningTrapsLoop mode={mode} onChangeMode={setMode} />;
}

function OpeningTrapsLoop({
  mode,
  onChangeMode,
}: {
  mode: AttemptMode;
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      setPuzzles(await api.listPuzzles("opening-traps"));
    } catch {
  ...
  const correctMove = result ? detailText(result, "correct_move") : null;
  const theme = result ? detailText(result, "theme") : null;
  const isCorrect = result?.result === "correct";

  return (
    <div>
      <PageHeader title={t("exercises.opening-traps.title")} subtitle={puzzle.prompt_fa} />
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <Badge>{task.side === "black" ? t("openingTraps.turnBlack") : t("openingTraps.turnWhite")}</Badge>
      </div>

      <Card>
        <p className="text-sm font-black text-stone-800">
          {t("openingTraps.trap")}: {task.trap}
        </p>
        <p className="mt-1 text-sm text-stone-500">
          {t("openingTraps.opening")}: {task.opening} · {t("openingTraps.theme")}: {task.themeFa}
        </p>
      </Card>

      <Card className="mt-3">
        <p className="text-sm font-black text-stone-800">{t("openingTraps.position")}</p>
        <p className="mt-2 text-base leading-8 text-stone-800">{task.description}</p>
        <p className="mt-3 text-xs text-stone-400">{t("openingTraps.audioSoon")}</p>
      </Card>

      {!result ? (
        <div>
          <p className="mt-3 text-base font-black text-stone-800">{t("openingTraps.answer")}</p>
          <p className="mt-1 text-sm text-stone-500">{t("openingTraps.instruction")}</p>
          <div className="mt-2">
            <input
              value={answer}
              ...
              spellCheck={false}
              disabled={submitting}
              aria-label={t("openingTraps.answer")}
              placeholder={t("openingTraps.answerPlaceholder")}
        ...
        <Card className="mt-3">
          <p className={`text-lg font-black ${isCorrect ? "text-green-700" : "text-red-600"}`}>
            {isCorrect ? t("openingTraps.correct") : t("openingTraps.wrong")}
          </p>
          {theme ? (
            <p className="mt-2 text-sm font-bold text-stone-700">
              {t("openingTraps.theme")}: {task.themeFa}
            </p>
          ) : null}
          {correctMove ? (
            <p className="mt-2 text-sm font-bold">
              {t("openingTraps.correctMove")}:{" "}

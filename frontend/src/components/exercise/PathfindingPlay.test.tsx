...
const ROOK_A1_FEN = "8/8/8/8/8/8/8/R7 w - - 0 1";
...
function puzzle(
  ...
  return {
    id,
    exercise_slug: "pathfinding",
    fen,
    position_json: { from, target, piece },
    hint_json: { hints: [] },
    prompt_fa: prompt,
    explanation: "",
    initial_rating: 900,
...
function renderPage(mode: string) {
  return render(
    <MemoryRouter initialEntries={[`/exercises/pathfinding?mode=${mode}`]}>
      <Routes>
        <Route path="/exercises/pathfinding" element={<PathfindingPage />} />
      </Routes>
    </MemoryRouter>,
  );
}
...
function mockStepOk(target: string) {
  mockedApi.validatePathStep = vi.fn().mockImplementation(
...
beforeEach(() => {
  vi.resetAllMocks();
  vi.useRealTimers();
  mockedApi.finishPathfindingSpeedSession = vi.fn().mockResolvedValue({
    session_id: "none",
    exercise_slug: "pathfinding",
    status: "finished",
    duration_s: 60,
...
describe("pure helpers", () => {
  ...
  it("posOf reads public task data", () => {
    expect(posOf(puzzle(1, "x"))).toEqual({ from: "a1", target: "a8", piece: "rook" });
  });
...
describe("catalog direct entry", () => {
  it("pathfinding exposes practice and speed entries", () => {
    const meta = getExerciseMeta("pathfinding");
    expect(meta?.status).toBe("active");
    expect(meta?.route).toBe("/exercises/pathfinding");
  ...
  it("mode entries point at distinct mode URLs", () => {
    ...
    expect(urls).toEqual(
      new Set(["/exercises/pathfinding?mode=practice", "/exercises/pathfinding?mode=speed"]),
    );
  });
});
...
describe("practice movement", () => {
  it("?mode=practice enters practice directly with star + selected piece", async () => {
    mockedApi.nextPathfindingPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "به ستاره برسان؟"));
    renderPage("practice");
  ...
  it("click destination moves the piece and keeps it selected", async () => {
    mockBoardRect();
    mockedApi.nextPathfindingPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "کلیک؟"));
    mockStepOk("a8");
    renderPage("practice");
    ...
    try {
      tap("a4");
      await act(async () => {});
      expect(mockedApi.validatePathStep).toHaveBeenCalledWith(
  ...
  it("drag moves the piece too (touch)", async () => {
    mockBoardRect();
    mockedApi.nextPathfindingPracticePuzzle = vi
      .fn()
  ...
  it("repeated clicks walk step by step without reselecting", async () => {
    mockBoardRect();
    mockedApi.nextPathfindingPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "قدم؟"));
    mockStepOk("a8");
    renderPage("practice");
    ...
    try {
      ...
      expect(mockedApi.validatePathStep).toHaveBeenCalledTimes(2);
  ...
  it("tapping the selected piece again keeps it selected", async () => {
    mockedApi.nextPathfindingPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "خودش؟"));
    renderPage("practice");
    expect(await screen.findByText("خودش؟")).toBeTruthy();
    tap("a1");
    await act(async () => {});
    expect(mockedApi.validatePathStep).not.toHaveBeenCalled();
    expect(screen.getByRole("gridcell", { name: "a1" }).getAttribute("aria-pressed")).toBe("true");
  });

  it("illegal destination buzzes, flashes red, stays, counts -3 state", async () => {
    mockBoardRect();
    mockedApi.nextPathfindingPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "خطا؟"));
    mockedApi.validatePathStep = vi.fn().mockResolvedValue({
      ok: false,
      fen: ROOK_A1_FEN,
      selected_at: "a1",
      reached: false,
  ...
  it("reaching the star submits path + illegal count, never scores", async () => {
    mockBoardRect();
    mockedApi.nextPathfindingPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "پایان؟"));
    mockStepOk("a8");
...
describe("speed mode", () => {
  function mockActiveBoot(batch: Puzzle[], sessionId: string) {
    mockedApi.startPathfindingSpeedSession = vi.fn().mockResolvedValue({
    ...
    mockedApi.preparePathfindingSpeedPuzzles = vi.fn().mockResolvedValue(batch);
    mockedApi.startPathfindingSpeedClock = vi.fn().mockResolvedValue({
      session_id: sessionId,
      exercise_slug: "pathfinding",
      status: "active",
      duration_s: 60,
  ...
  function mockSubmitOk(sessionId: string) {
    let n = 0;
    mockedApi.submitPathfindingSpeedAnswer = vi.fn().mockImplementation(
  ...
  it("?mode=speed prepares a 20-puzzle buffer before starting the clock", async () => {
    ...
    expect(mockedApi.preparePathfindingSpeedPuzzles).toHaveBeenCalledWith("s1", { count: 20 });
    expect(mockedApi.startPathfindingSpeedClock).toHaveBeenCalledWith("s1");
  ...
  it("speed reaches the star and auto-advances with no manual next", async () => {
    ...
    try {
      ...
      await act(async () => {});
      expect(mockedApi.submitPathfindingSpeedAnswer).toHaveBeenCalledWith(
        "chain1",
        expect.objectContaining({
          puzzle_id: 1,
          answer: { path: ["a1", "a8"], illegal_attempts: 0 },
  ...
  it("speed illegal move buzzes and keeps the puzzle going", async () => {
    ...
    try {
      ...
      expect(mockedApi.submitPathfindingSpeedAnswer).not.toHaveBeenCalled();
      expect(screen.getByRole("gridcell", { name: "a1" }).getAttribute("aria-pressed")).toBe("true");
    } finally {
      vi.useRealTimers();
    }

...
// Shared-position style: white rook e4 attacked by the black bishop c6,
// undefended; the whole board is the question (no hunter square).
const FEN = "4k3/8/2b5/8/4R3/8/8/4K3 w - - 0 1";

function puzzle(id: number, prompt: string, withHint = false): Puzzle {
  return {
    id,
    exercise_slug: "undefended-pieces",
    fen: FEN,
...
function renderPage(mode: string) {
  return render(
    <MemoryRouter initialEntries={[`/exercises/undefended-pieces?mode=${mode}`]}>
      <Routes>
        <Route path="/exercises/undefended-pieces" element={<UndefendedPiecesPage />} />
      </Routes>
    </MemoryRouter>,
  );
}
...
beforeEach(() => {
  ...
  mockedApi.finishUndefendedSpeedSession = vi.fn().mockResolvedValue({
    session_id: "none",
    exercise_slug: "undefended-pieces",
    status: "finished",
    duration_s: 60,
...
describe("catalog direct entry", () => {
  it("undefended-pieces exposes practice and speed entries", () => {
    const meta = getExerciseMeta("undefended-pieces");
    expect(meta?.status).toBe("active");
    expect(meta?.route).toBe("/exercises/undefended-pieces");
  ...
  it("mode entries point at distinct mode URLs", () => {
    ...
    expect(urls).toEqual(
      new Set(["/exercises/undefended-pieces?mode=practice", "/exercises/undefended-pieces?mode=speed"]),
    );
  });
});
...
describe("mode navigation", () => {
  it("?mode=practice enters practice directly with the first puzzle", async () => {
    mockedApi.nextUndefendedPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "سؤال تمرینی؟"));
    renderPage("practice");
  ...
  it("?mode=speed prepares a 20-puzzle buffer before starting the clock", async () => {
    ...
    mockedApi.startUndefendedSpeedSession = vi.fn().mockResolvedValue({
      session_id: "s1",
      exercise_slug: "undefended-pieces",
      status: "preparing",
      duration_s: 60,
    ...
    mockedApi.prepareUndefendedSpeedPuzzles = vi.fn().mockResolvedValue(batch);
    mockedApi.startUndefendedSpeedClock = vi.fn().mockResolvedValue({
      session_id: "s1",
      exercise_slug: "undefended-pieces",
      status: "active",
      duration_s: 60,
    ...
    expect(mockedApi.prepareUndefendedSpeedPuzzles).toHaveBeenCalledWith("s1", { count: 20 });
    expect(mockedApi.startUndefendedSpeedClock).toHaveBeenCalledWith("s1");
    expect(await screen.findByText("زمان باقی‌مانده")).toBeTruthy();
  });
});

describe("selection without pre-highlight", () => {
  it("nothing is highlighted before submission and answers are not revealed", async () => {
    const user = userEvent.setup();
    mockedApi.nextUndefendedPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "سؤال هدف؟"));
    ...
    expect(screen.getByRole("gridcell", { name: "e4" }).className).not.toContain("outline-sky-400");
    expect(screen.getByRole("gridcell", { name: "c6" }).className).not.toContain("outline-sky-400");
  ...
  it("feedback colors distinguish correct/missed/wrong", async () => {
    const user = userEvent.setup();
    mockedApi.nextUndefendedPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "سؤال رنگ؟"));
    mockedApi.submitAttempt = vi.fn().mockResolvedValue(attempt(1, "partial", 7));
    renderPage("practice");
  ...
  it("empty selection submits and zero-target feedback shows +5", async () => {
    const user = userEvent.setup();
    mockedApi.nextUndefendedPracticePuzzle = vi.fn().mockResolvedValue(puzzle(9, "سؤال خالی؟"));
  ...
  it("practice progresses to the prefetched next puzzle", async () => {
    const user = userEvent.setup();
    mockedApi.nextUndefendedPracticePuzzle = vi
      .fn()
...
describe("speed consecutive submissions", () => {
  ...
  function mockSubmitOk(sessionId: string) {
    let n = 0;
    mockedApi.submitUndefendedSpeedAnswer = vi.fn().mockImplementation(async (_sid: string, body: { puzzle_id: number }) => {
      n += 1;
      return {
  ...
  it("three consecutive puzzles each submit with fresh state", async () => {
    ...
    try {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        await act(async () => {});
        expect(mockedApi.submitUndefendedSpeedAnswer).toHaveBeenCalledTimes(i);
        expect(mockedApi.submitUndefendedSpeedAnswer).toHaveBeenLastCalledWith(
          "chain1",
  ...
  it("background refill keeps play continuous without loading states", async () => {
    ...
    try {
      ...
      renderPage("speed");
      await flushBoot();
      expect(mockedApi.prepareUndefendedSpeedPuzzles).toHaveBeenCalledTimes(1);
      ...
      expect(mockedApi.prepareUndefendedSpeedPuzzles).toHaveBeenCalledTimes(2);
      expect(mockedApi.prepareUndefendedSpeedPuzzles).toHaveBeenLastCalledWith("refill1", { count: 12 });
      expect(screen.getByText("زمان باقی‌مانده")).toBeTruthy();
    } finally {
      vi.useRealTimers();
    }
  ...
  it("preparing UI hides internal buffer counts", async () => {
    ...
    try {
      ...
      mockedApi.prepareUndefendedSpeedPuzzles = vi.fn().mockReturnValue(gate);
      mockedApi.startUndefendedSpeedClock = vi.fn().mockResolvedValue({
        session_id: "gate1",
        exercise_slug: "undefended-pieces",
        status: "active",
...
describe("speed expiry and report", () => {
  it("expired session shows the authoritative per-puzzle report", async () => {
    ...
    try {
      ...
      mockedApi.submitUndefendedSpeedAnswer = vi.fn().mockResolvedValue({
      ...
      mockedApi.finishUndefendedSpeedSession = vi.fn().mockResolvedValue(report.session);
      mockedApi.getUndefendedSpeedReport = vi.fn().mockResolvedValue(report);
      ...
      fireEvent.click(screen.getByRole("button", { name: "بررسی جواب" }));
      await act(async () => {});
      expect(mockedApi.submitUndefendedSpeedAnswer).toHaveBeenCalledTimes(1);
      expect(screen.getByText("آفرین! درست بود.")).toBeTruthy();
      expect(screen.queryByRole("button", { name: "معمای بعدی" })).toBeNull();
      await act(async () => {
        await vi.advanceTimersByTimeAsync(500);
      ...
      expect(mockedApi.getUndefendedSpeedReport).toHaveBeenCalledWith("s9");
    } finally {
      vi.useRealTimers();
    }
  });
...
describe("gameplay chrome", () => {
  it("practice shows question, board, and submit together", async () => {
    const user = userEvent.setup();
    mockedApi.nextUndefendedPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "سؤال چینش؟"));
    renderPage("practice");
  ...
  it("hint is a compact ؟ button that reveals help without navigation", async () => {
    const user = userEvent.setup();
    mockedApi.nextUndefendedPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "سؤال راهنما؟", true));

...
// White-only position: white knight on d4, white king on e1.
const FEN = "8/8/8/8/3N4/8/8/4K3 w - - 0 1";
...
function renderPage(mode: string) {
  return render(
    <MemoryRouter initialEntries={[`/exercises/legal-destinations?mode=${mode}`]}>
      <Routes>
        <Route path="/exercises/legal-destinations" element={<LegalDestinationsPage />} />
      </Routes>
    </MemoryRouter>,
  );
}
...
beforeEach(() => {
  ...
  mockedApi.finishLegalSpeedSession = vi.fn().mockResolvedValue({
    session_id: "none",
    exercise_slug: "legal-destinations",
    status: "finished",
    duration_s: 60,
...
describe("catalog direct entry", () => {
  it("legal-destinations exposes practice and speed entries", () => {
    const meta = getExerciseMeta("legal-destinations");
    expect(meta?.status).toBe("active");
    expect(meta?.route).toBe("/exercises/legal-destinations");
  ...
  it("mode entries point at distinct mode URLs", () => {
    ...
    expect(urls).toEqual(
      new Set(["/exercises/legal-destinations?mode=practice", "/exercises/legal-destinations?mode=speed"]),
    );
  });
});
...
describe("mode navigation", () => {
  it("?mode=practice enters practice directly with the first puzzle", async () => {
    mockedApi.nextLegalPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "سؤال تمرینی؟"));
    renderPage("practice");
  ...
  it("?mode=speed prepares a 20-puzzle buffer before starting the clock", async () => {
    ...
    mockedApi.startLegalSpeedSession = vi.fn().mockResolvedValue({
      session_id: "s1",
      exercise_slug: "legal-destinations",
      status: "preparing",
      duration_s: 60,
    ...
    mockedApi.prepareLegalSpeedPuzzles = vi.fn().mockResolvedValue(batch);
    mockedApi.startLegalSpeedClock = vi.fn().mockResolvedValue({
      session_id: "s1",
      exercise_slug: "legal-destinations",
      status: "active",
      duration_s: 60,
    ...
    expect(mockedApi.prepareLegalSpeedPuzzles).toHaveBeenCalledWith("s1", { count: 20 });
    expect(mockedApi.startLegalSpeedClock).toHaveBeenCalledWith("s1");
...
describe("target highlight and selection", () => {
  it("target piece is highlighted before submission without revealing answers", async () => {
    ...
    mockedApi.nextLegalPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "سؤال هدف؟"));
  ...
  it("feedback colors distinguish correct/missed/wrong", async () => {
    const user = userEvent.setup();
    mockedApi.nextLegalPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "سؤال رنگ؟"));
    mockedApi.submitAttempt = vi.fn().mockResolvedValue(attempt(1, "partial", 7));
    renderPage("practice");
  ...
  it("empty selection submits and zero-target feedback shows +5", async () => {
    const user = userEvent.setup();
    mockedApi.nextLegalPracticePuzzle = vi.fn().mockResolvedValue(puzzle(9, "سؤال خالی؟"));
  ...
  it("practice progresses to the prefetched next puzzle", async () => {
    const user = userEvent.setup();
    mockedApi.nextLegalPracticePuzzle = vi
      .fn()
...
describe("speed consecutive submissions", () => {
  ...
  function mockSubmitOk(sessionId: string) {
    let n = 0;
    mockedApi.submitLegalSpeedAnswer = vi.fn().mockImplementation(async (_sid: string, body: { puzzle_id: number }) => {
      n += 1;
      return {
  ...
  it("three consecutive puzzles each submit with fresh state", async () => {
    ...
    try {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        await act(async () => {});
        expect(mockedApi.submitLegalSpeedAnswer).toHaveBeenCalledTimes(i);
        expect(mockedApi.submitLegalSpeedAnswer).toHaveBeenLastCalledWith(
          "chain1",
  ...
  it("background refill keeps play continuous without loading states", async () => {
    ...
    try {
      ...
      renderPage("speed");
      await flushBoot();
      expect(mockedApi.prepareLegalSpeedPuzzles).toHaveBeenCalledTimes(1);
      ...
      expect(mockedApi.prepareLegalSpeedPuzzles).toHaveBeenCalledTimes(2);
      expect(mockedApi.prepareLegalSpeedPuzzles).toHaveBeenLastCalledWith("refill1", { count: 12 });
      expect(screen.getByText("زمان باقی‌مانده")).toBeTruthy();
    } finally {
      vi.useRealTimers();
    }
  ...
  it("preparing UI hides internal buffer counts", async () => {
    ...
    try {
      ...
      mockedApi.prepareLegalSpeedPuzzles = vi.fn().mockReturnValue(gate);
      mockedApi.startLegalSpeedClock = vi.fn().mockResolvedValue({
        session_id: "gate1",
        exercise_slug: "legal-destinations",
        status: "active",
...
describe("speed expiry and report", () => {
  it("expired session shows the authoritative per-puzzle report", async () => {
    ...
    try {
      ...
      mockedApi.submitLegalSpeedAnswer = vi.fn().mockResolvedValue({
      ...
      mockedApi.finishLegalSpeedSession = vi.fn().mockResolvedValue(report.session);
      mockedApi.getLegalSpeedReport = vi.fn().mockResolvedValue(report);
      ...
      fireEvent.click(screen.getByRole("button", { name: "بررسی جواب" }));
      await act(async () => {});
      expect(mockedApi.submitLegalSpeedAnswer).toHaveBeenCalledTimes(1);
      expect(screen.getByText("آفرین! درست بود.")).toBeTruthy();
      expect(screen.queryByRole("button", { name: "معمای بعدی" })).toBeNull();
      await act(async () => {
        await vi.advanceTimersByTimeAsync(500);
      ...
      expect(mockedApi.getLegalSpeedReport).toHaveBeenCalledWith("s9");
    } finally {
      vi.useRealTimers();
    }
  });
...
describe("gameplay chrome", () => {
  it("practice shows question, board, and submit together", async () => {
    const user = userEvent.setup();
    mockedApi.nextLegalPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "سؤال چینش؟"));
    renderPage("practice");
  ...
  it("hint is a compact ؟ button that reveals help without navigation", async () => {
    const user = userEvent.setup();
    mockedApi.nextLegalPracticePuzzle = vi.fn().mockResolvedValue(puzzle(1, "سؤال راهنما؟", true));

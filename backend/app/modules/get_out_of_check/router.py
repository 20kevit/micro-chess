"""Get Out of Check routes: practice next-puzzle + speed sessions.
...
router = APIRouter(prefix="/get-out-of-check", tags=["get-out-of-check"])


def _to_out(session: sessions.GetOutOfCheckSpeedSession) -> schemas.SessionOut:
    data = sessions.summary(session)
    return schemas.SessionOut(**data)


def _to_summary(session: sessions.GetOutOfCheckSpeedSession) -> schemas.SessionSummary:

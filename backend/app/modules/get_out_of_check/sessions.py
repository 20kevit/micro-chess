"""Get Out of Check speed sessions: prepared buffer + authoritative clock.
...
def _expire_if_needed(db: Session, session: GetOutOfCheckSpeedSession) -> GetOutOfCheckSpeedSession:
    if (
...
def get_session(db: Session, session_id: str) -> GetOutOfCheckSpeedSession:
    session = db.get(GetOutOfCheckSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> GetOutOfCheckSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> GetOutOfCheckSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> GetOutOfCheckSpeedSession:
...
def submit(
    ...
    *,
    ...
    # Client-supplied FEN/solution metadata is ignored: validation
...
def finish(db: Session, session_id: str) -> GetOutOfCheckSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: GetOutOfCheckSpeedSession) -> dict[str, Any]:

"""Undefended Pieces routes: practice next-puzzle + speed sessions.
...
from app.modules.undefended_pieces import schemas, sessions
from app.modules.undefended_pieces.validator import SLUG
from app.modules.undefended_pieces.sessions import (
    BufferNotReadyError,
...
router = APIRouter(prefix="/undefended-pieces", tags=["undefended-pieces"])


def _to_out(session: sessions.UndefendedSpeedSession) -> schemas.SessionOut:
    data = sessions.summary(session)
    return schemas.SessionOut(**data)


def _to_summary(session: sessions.UndefendedSpeedSession) -> schemas.SessionSummary:

"""Undefended Pieces speed sessions: prepared buffer + authoritative clock.
...
from app.modules.undefended_pieces import generator
from app.modules.undefended_pieces.models import UndefendedSpeedSession
from app.modules.undefended_pieces.validator import SLUG
...
def _expire_if_needed(db: Session, session: UndefendedSpeedSession) -> UndefendedSpeedSession:
    if (
...
def get_session(db: Session, session_id: str) -> UndefendedSpeedSession:
    session = db.get(UndefendedSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> UndefendedSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> UndefendedSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> UndefendedSpeedSession:
...
def finish(db: Session, session_id: str) -> UndefendedSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: UndefendedSpeedSession) -> dict[str, Any]:

...
Memorization rule: ``memorization_ms = piece_count * 300``. The count is
naturally bounded by chess itself (at most 32 pieces on a legal board, so
at most 9600 ms); no artificial difficulty filter is applied. Measured on
...
1.2s-9.6s of study time. Every real position is servable as-is.
"""
...
# Memorization budget per piece (0.3 seconds, spec section 4).
MEMORIZE_MS_PER_PIECE = 300
...
def memorization_ms(fen: str) -> int:
    """Authoritative study budget: ``piece_count * 300`` ms.

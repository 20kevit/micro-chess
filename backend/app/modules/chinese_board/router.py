"""Chinese Board routes: practice next-puzzle + speed sessions.
...
(``memorization_ms = piece_count * 300``) is computed server-side.
"""
...
from app.modules.chinese_board import schemas, sessions
from app.modules.chinese_board.sessions import (
    BufferNotReadyError,
...
router = APIRouter(prefix="/chinese-board", tags=["chinese-board"])


def _to_out(session: sessions.ChineseBoardSpeedSession) -> schemas.SessionOut:
    data = sessions.summary(session)
    return schemas.SessionOut(**data)


def _to_summary(session: sessions.ChineseBoardSpeedSession) -> schemas.SessionSummary:
...
def next_practice_puzzle(
    ...
    """Issue one fresh random real position for untimed Practice Mode."""

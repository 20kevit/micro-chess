"""Chinese Board speed sessions: prepared buffer + authoritative clock.
...
- The frontend runs a visible countdown for UX, but ONLY this service
  ...
  against the stored ``ends_at`` and rejects late answers. The per-puzzle
  memorization budget (``position_json.memorization_ms``) is a display
  budget only; grading always revalidates the full reconstruction
  server-side from the stored FEN.
...
from app.modules.chinese_board import generator
from app.modules.chinese_board.models import ChineseBoardSpeedSession
from app.modules.chinese_board.validator import SLUG, normalize_entry
...
def _expire_if_needed(db: Session, session: ChineseBoardSpeedSession) -> ChineseBoardSpeedSession:
    if session.status == "active" and session.ends_at is not None and _now() >= session.ends_at:
...
def get_session(db: Session, session_id: str) -> ChineseBoardSpeedSession:
    session = db.get(ChineseBoardSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> ChineseBoardSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> ChineseBoardSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> ChineseBoardSpeedSession:
...
def _safe_answer(answer: Any) -> dict[str, Any]:
    """Keep only the submitted pieces; ignore client totals/scores/FENs."""
    if not isinstance(answer, dict):
        return {"pieces": []}
    raw = answer.get("pieces")
    cleaned: list[dict[str, str]] = []
    if isinstance(raw, list):
        for entry in raw:
            item = normalize_entry(entry)
            if item is None:
                # Fail-safe: malformed entries are dropped here; the
                # validator still grades what remains (missing pieces
                # count as errors, so guessing cannot profit).
                continue
            side, kind, square = item
            cleaned.append({"square": square, "piece": kind, "color": side})
...
def submit(
    ...
    *,
    ...
    # Client-supplied FEN/counts/score metadata is ignored: validation
...
def finish(db: Session, session_id: str) -> ChineseBoardSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: ChineseBoardSpeedSession) -> dict[str, Any]:
...
def report(db: Session, session_id: str) -> dict[str, Any]:
    ...
    Piece counts are re-derived server-side by re-running the exercise
    ...
    if attempt_ids:
        attempts = db.query(Attempt).filter(Attempt.id.in_(attempt_ids)).order_by(Attempt.id).all()
        puzzles = {
            p.id: p for p in db.query(Puzzle).filter(Puzzle.id.in_([a.puzzle_id for a in attempts]))
        }
            ...
            entries.append(
                {
                    ...
                    "missed": list(detail.get("missed", detail.get("missing", []))),

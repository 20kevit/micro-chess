"""Pathfinding speed sessions: prepared buffer + authoritative clock.
...
def _expire_if_needed(db: Session, session: PathfindingSpeedSession) -> PathfindingSpeedSession:
...
def get_session(db: Session, session_id: str) -> PathfindingSpeedSession:
    session = db.get(PathfindingSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> PathfindingSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> PathfindingSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> PathfindingSpeedSession:
...
def finish(db: Session, session_id: str) -> PathfindingSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: PathfindingSpeedSession) -> dict[str, Any]:

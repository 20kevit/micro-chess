"""Obstacle-pathfinding speed sessions: prepared buffer + authoritative clock.

Lifecycle: ``preparing`` -> ``active`` -> ``finished``/``expired``,
mirroring Exercise 6 (same columns, separate table):
...
def _expire_if_needed(
    db: Session, session: ObstaclePathfindingSpeedSession
) -> ObstaclePathfindingSpeedSession:
...
def start_session(
    ...
    *,
    ...
    started = _now()
    session = ObstaclePathfindingSpeedSession(
        id=uuid.uuid4().hex,
        exercise_slug=SLUG,
        user_id=user_id,
        status="preparing",
...
def get_session(db: Session, session_id: str) -> ObstaclePathfindingSpeedSession:
    session = db.get(ObstaclePathfindingSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> ObstaclePathfindingSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> ObstaclePathfindingSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> ObstaclePathfindingSpeedSession:
...
def _safe_answer(answer: Any) -> dict[str, Any]:
    """Keep only the path + illegal counter; ignore client score/optimals."""
    if not isinstance(answer, dict):
        return {}
    path = answer.get("path")
    illegal = answer.get("illegal_attempts")
    safe: dict[str, Any] = {}
    if isinstance(path, list):
        safe["path"] = [p for p in path if isinstance(p, str)][:64]
    else:
        safe["path"] = []
    safe["illegal_attempts"] = illegal if isinstance(illegal, int) and illegal >= 0 else 0
    return safe
...
def finish(db: Session, session_id: str) -> ObstaclePathfindingSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: ObstaclePathfindingSpeedSession) -> dict[str, Any]:
...
def report(db: Session, session_id: str) -> dict[str, Any]:
    ...
    Move counts are re-derived server-side by re-running the exercise

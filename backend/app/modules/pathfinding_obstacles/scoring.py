"""Pathfinding-with-obstacles scoring (Exercise 7).

score = optimal_moves * 5 - max(0, actual_moves - optimal_moves) * 2
        - illegal_attempts * 3

Same formula as Exercise 6 (the exercise is its direct evolution):
- ``optimal_moves`` is the server-computed BFS minimum over complete
  board states (never trusted from the client); ``actual_moves`` =
  len(path) - 1 from the validated path (captures count as one legal
  move each); ``illegal_attempts`` is clamped to >= 0 server-side.
...
- Scores may be negative; nothing clamps them here (project-wide policy,
  same as Exercises 1-6). The result label (CORRECT/WRONG) is computed
  independently by the validator.
"""
...
def score_obstacle_path(validation: ValidationResult) -> float:

"""Pathfinding-with-obstacles speed-session model (60-second sessions).

Mirrors the Exercise 6 table: individual answers flow through the
standard ``attempts`` table (mode=practice, so ``rating_delta`` stays
None per the rating-stub rule); this table stores the authoritative
session clock and aggregates. Portable column types only.
"""
...
class ObstaclePathfindingSpeedSession(Base):
    __tablename__ = "pathfinding_obstacles_speed_sessions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    exercise_slug: Mapped[str] = mapped_column(String(100), default="pathfinding-obstacles", index=True)

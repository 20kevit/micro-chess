"""Blindfold Square Vision speed sessions: prepared buffer + authoritative clock.
...
from app.modules.blindfold_square_vision import generator
from app.modules.blindfold_square_vision.models import BlindfoldSquareVisionSpeedSession
from app.modules.blindfold_square_vision.validator import SLUG, normalize_choice, normalize_square
...
def _expire_if_needed(
    db: Session, session: BlindfoldSquareVisionSpeedSession
) -> BlindfoldSquareVisionSpeedSession:
...
def start_session(
    ...
    *,
    ...
    started = _now()
    session = BlindfoldSquareVisionSpeedSession(
        id=uuid.uuid4().hex,
        exercise_slug=SLUG,
        user_id=user_id,
        status="preparing",
...
def get_session(db: Session, session_id: str) -> BlindfoldSquareVisionSpeedSession:
    session = db.get(BlindfoldSquareVisionSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> BlindfoldSquareVisionSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> BlindfoldSquareVisionSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> BlindfoldSquareVisionSpeedSession:
...
def _safe_answer(answer: Any) -> dict[str, Any]:
    """Keep only the submitted square/choice; ignore everything else."""
    if not isinstance(answer, dict):
        return {"choice": ""}
    square = normalize_square(answer.get("square"))
...
def submit(
    ...
    *,
    ...
    # Client-supplied colors/scores are ignored: validation always runs
    # against the stored server-side answer_json.
...
def finish(db: Session, session_id: str) -> BlindfoldSquareVisionSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: BlindfoldSquareVisionSpeedSession) -> dict[str, Any]:
...
def report(db: Session, session_id: str) -> dict[str, Any]:
    ...
    Answers are re-derived server-side by re-running the exercise

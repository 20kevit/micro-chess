"""Blindfold Square Vision routes: practice next-puzzle + speed sessions.

Thin wiring only; generation/clock/scoring live in generator/sessions and
the shared progress service. The square color is never exposed:
...
from app.modules.blindfold_square_vision import schemas, sessions
from app.modules.blindfold_square_vision.validator import SLUG
from app.modules.blindfold_square_vision.sessions import (
    BufferNotReadyError,
...
router = APIRouter(prefix="/blindfold-square-vision", tags=["blindfold-square-vision"])


def _to_out(session: sessions.BlindfoldSquareVisionSpeedSession) -> schemas.SessionOut:
    data = sessions.summary(session)
    return schemas.SessionOut(**data)


def _to_summary(session: sessions.BlindfoldSquareVisionSpeedSession) -> schemas.SessionSummary:
...
def next_practice_puzzle(
    ...
    """Issue one fresh uniform-random square puzzle for untimed Practice Mode."""

"""Balance Scale speed sessions: prepared buffer + authoritative clock.
...
from app.modules.balance_scale import generator
from app.modules.balance_scale.models import BalanceScaleSpeedSession
from app.modules.balance_scale.validator import SLUG, normalize_piece
...
def _expire_if_needed(db: Session, session: BalanceScaleSpeedSession) -> BalanceScaleSpeedSession:
...
def get_session(db: Session, session_id: str) -> BalanceScaleSpeedSession:
    session = db.get(BalanceScaleSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> BalanceScaleSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> BalanceScaleSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> BalanceScaleSpeedSession:
...
def _safe_answer(answer: Any) -> dict[str, Any]:
    """Keep only the submitted pieces; ignore client score/optimal/target."""
    if not isinstance(answer, dict):
        return {"pieces": []}
    raw = answer.get("pieces")
    pieces = [p for p in raw if isinstance(p, str) and normalize_piece(p) is not None][:16]
...
def submit(
    ...
    *,
    ...
    # Client-supplied score/optimal/path-override metadata is ignored:
    # validation always runs against the stored server-side answer_json.
...
def finish(db: Session, session_id: str) -> BalanceScaleSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: BalanceScaleSpeedSession) -> dict[str, Any]:
...
def report(db: Session, session_id: str) -> dict[str, Any]:
    ...
    Counts are re-derived server-side by re-running the exercise

"""Give Check validator: play any legal move that checks the opponent king.

Puzzle definition (stored in the puzzle row, never exposed to clients):
- answer_json: {"fen": "...", "mode": "all-checks"|"appropriate-checks",
  "example": {"from": "e2", "to": "e4"}}. The FEN must live here because
  validators only receive answer_json, and correctness is always recomputed
  from the position — never from a stored move list.
...
(python-chess: pins, discovered checks, captures and king safety included)
and the opponent king is in check afterwards. Any legal checking move
counts, so positions with several solutions accept them all.

Modes ("all-checks" vs "appropriate-checks") currently behave identically
by design; the mode travels in the data model so future logic can diverge
without redesigning the exercise.
...
MODES = ("all-checks", "appropriate-checks")

_PROMOTIONS = {"q": chess.QUEEN, "r": chess.ROOK, "b": chess.BISHOP, "n": chess.KNIGHT}


def checking_moves(fen: str) -> list[str]:
    """All legal checking moves in UCI form (used by seed verification + tests)."""
...
def is_checking_move(fen: str, from_sq: str, to_sq: str, promotion: Any = None) -> bool:
    """True when from→to is legal and checks the opponent king.

    Raises ValueError on invalid FEN or squares. An explicit promotion
    restricts to that choice; without one, any checking promotion counts.
    """
    board = chess.Board(fen)  # raises on invalid FEN
...
def validate(puzzle_answer: dict[str, Any], attempt: dict[str, Any]) -> ValidationResult:
    ...
    try:
        correct = is_checking_move(fen, origin, dest, promotion)
    except ValueError:
        correct = False
    if correct:
        return ValidationResult(

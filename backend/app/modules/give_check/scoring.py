"""Giving Check scoring: per-move, server-authoritative.
...
(empty checking-move set, no arrows submitted) awards a +5 completion
bonus instead of 0, so finding "nothing checks" still feels rewarding.
...
- Counts come from the validation detail (backend-computed, never trusted
  ...
  and cost -2 each, like any other wrong arrow.

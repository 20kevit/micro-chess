"""Blindfold Calculation routes: practice next-puzzle + speed sessions.

Thin wiring only; generation/clock/scoring live in generator/sessions and
the shared progress service. The FEN and the solution move are never
exposed: ``next`` endpoints return ``PuzzleOut`` (no ``answer_json``).
"""
...
from app.modules.blindfold_calculation import schemas, sessions
from app.modules.blindfold_calculation.sessions import (
    BufferNotReadyError,
...
router = APIRouter(prefix="/blindfold-calculation", tags=["blindfold-calculation"])


def _to_out(session: sessions.BlindfoldCalculationSpeedSession) -> schemas.SessionOut:
    data = sessions.summary(session)
    return schemas.SessionOut(**data)


def _to_summary(session: sessions.BlindfoldCalculationSpeedSession) -> schemas.SessionSummary:
...
def next_practice_puzzle(
    ...
    """Issue one fresh random eligible puzzle for untimed Practice Mode."""

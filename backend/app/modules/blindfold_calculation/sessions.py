"""Blindfold Calculation speed sessions: prepared buffer + authoritative clock.
...
- Scoring reuses the standard ``progress.service.submit_attempt``
  (mode=practice) plus the shared default scorer — no separate scoring
  system. Per-answer rows live in ``attempts``; this table only
...
def _expire_if_needed(
    db: Session, session: BlindfoldCalculationSpeedSession
) -> BlindfoldCalculationSpeedSession:
...
def start_session(
    ...
    *,
    ...
    started = _now()
    session = BlindfoldCalculationSpeedSession(
        id=uuid.uuid4().hex,
        exercise_slug=SLUG,
        user_id=user_id,
        status="preparing",
...
def get_session(db: Session, session_id: str) -> BlindfoldCalculationSpeedSession:
    session = db.get(BlindfoldCalculationSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> BlindfoldCalculationSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> BlindfoldCalculationSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> BlindfoldCalculationSpeedSession:
...
def _safe_answer(answer: Any) -> dict[str, Any]:
    """Keep only the submitted SAN move; ignore client FEN/score/timing."""
    if not isinstance(answer, dict):
        return {"move": ""}
    raw = answer.get("move")
    return {"move": raw.strip() if isinstance(raw, str) else ""}
...
def finish(db: Session, session_id: str) -> BlindfoldCalculationSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: BlindfoldCalculationSpeedSession) -> dict[str, Any]:
...
def report(db: Session, session_id: str) -> dict[str, Any]:
    ...
    SANs are re-derived server-side by re-running the exercise validator
    over the stored puzzle answer + stored user answer (never trusted
    from any client payload). ``Puzzle.fen`` stays NULL for this
    exercise, so entries never leak positions.
    """

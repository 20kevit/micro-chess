"""Captures speed sessions: prepared buffer + authoritative clock.
...
from app.modules.captures import generator
from app.modules.captures.models import CaptureSpeedSession
from app.modules.captures.validator import SLUG
...
def _expire_if_needed(db: Session, session: CaptureSpeedSession) -> CaptureSpeedSession:
    if (
...
def get_session(db: Session, session_id: str) -> CaptureSpeedSession:
    session = db.get(CaptureSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> CaptureSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> CaptureSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> CaptureSpeedSession:
...
def finish(db: Session, session_id: str) -> CaptureSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: CaptureSpeedSession) -> dict[str, Any]:

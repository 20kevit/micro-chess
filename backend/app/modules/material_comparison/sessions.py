"""Heavier Side speed sessions: prepared buffer + authoritative clock.
...
from app.modules.material_comparison import generator
from app.modules.material_comparison.models import HeavierSideSpeedSession
from app.modules.material_comparison.validator import SLUG, normalize_choice
...
def _expire_if_needed(db: Session, session: HeavierSideSpeedSession) -> HeavierSideSpeedSession:
...
def get_session(db: Session, session_id: str) -> HeavierSideSpeedSession:
    session = db.get(HeavierSideSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> HeavierSideSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> HeavierSideSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> HeavierSideSpeedSession:
...
def _safe_answer(answer: Any) -> dict[str, Any]:
    """Keep only the submitted choice; ignore client totals/scores."""
...
def submit(
    ...
    *,
    ...
    # Client-supplied FEN/totals/score metadata is ignored: validation
...
def finish(db: Session, session_id: str) -> HeavierSideSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: HeavierSideSpeedSession) -> dict[str, Any]:
...
def report(db: Session, session_id: str) -> dict[str, Any]:
    ...
    Choices are re-derived server-side by re-running the exercise

"""Heavier Side routes: practice next-puzzle + speed sessions.
...
from app.modules.material_comparison import schemas, sessions
from app.modules.material_comparison.sessions import (
    BufferNotReadyError,
...
router = APIRouter(prefix="/heavier-side", tags=["heavier-side"])


def _to_out(session: sessions.HeavierSideSpeedSession) -> schemas.SessionOut:
    data = sessions.summary(session)
    return schemas.SessionOut(**data)


def _to_summary(session: sessions.HeavierSideSpeedSession) -> schemas.SessionSummary:

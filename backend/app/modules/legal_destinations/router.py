"""Legal Destinations routes: practice next-puzzle + speed sessions.
...
from app.modules.legal_destinations import schemas, sessions
from app.modules.legal_destinations.validator import SLUG
from app.modules.legal_destinations.sessions import (
    BufferNotReadyError,
...
router = APIRouter(prefix="/legal-destinations", tags=["legal-destinations"])


def _to_out(session: sessions.LegalSpeedSession) -> schemas.SessionOut:
    data = sessions.summary(session)
    return schemas.SessionOut(**data)


def _to_summary(session: sessions.LegalSpeedSession) -> schemas.SessionSummary:

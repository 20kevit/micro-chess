"""Legal Destinations speed sessions: prepared buffer + authoritative clock.
...
def _expire_if_needed(db: Session, session: LegalSpeedSession) -> LegalSpeedSession:
    if (
...
def get_session(db: Session, session_id: str) -> LegalSpeedSession:
    session = db.get(LegalSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> LegalSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> LegalSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> LegalSpeedSession:
...
def finish(db: Session, session_id: str) -> LegalSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: LegalSpeedSession) -> dict[str, Any]:

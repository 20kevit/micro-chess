"""Trapped Pieces routes: practice next-puzzle + speed sessions.
...
from app.modules.trapped_pieces import schemas, sessions
from app.modules.trapped_pieces.sessions import (
    BufferNotReadyError,
...
router = APIRouter(prefix="/trapped-pieces", tags=["trapped-pieces"])


def _to_out(session: sessions.TrappedSpeedSession) -> schemas.SessionOut:
    data = sessions.summary(session)
    return schemas.SessionOut(**data)


def _to_summary(session: sessions.TrappedSpeedSession) -> schemas.SessionSummary:
...
def prepare_speed_puzzles(
    ...
    """Prepare ``count`` single-answer puzzles into the session buffer
    (initial >=20 before start, background refill while active). Never
    leaks answers."""
    try:

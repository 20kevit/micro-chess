"""Trapped Pieces speed sessions: prepared buffer + authoritative clock.
...
- Every buffered puzzle holds EXACTLY ONE trapped piece (the generator's
  ``exactly_one`` mode), so a single tap unambiguously grades +5/-4 and
  the Speed loop can submit immediately on selection.
...
from app.modules.trapped_pieces.models import TrappedSpeedSession
...
def _expire_if_needed(db: Session, session: TrappedSpeedSession) -> TrappedSpeedSession:
    if (
...
def get_session(db: Session, session_id: str) -> TrappedSpeedSession:
    session = db.get(TrappedSpeedSession, session_id)
    if session is None:
        raise SessionNotFoundError("session_not_found")
    return _expire_if_needed(db, session)


def _require_active(db: Session, session_id: str) -> TrappedSpeedSession:
...
def _require_preparable(db: Session, session_id: str) -> TrappedSpeedSession:
    session = get_session(db, session_id)
...
def begin_session(db: Session, session_id: str) -> TrappedSpeedSession:
...
def _attach_puzzles(
    ...
    for _ in range(max(0, count)):
        puzzle = generator.create_puzzle(db, rng, exclude_ids=excluded, exactly_one=True)
...
def prepare_puzzles(
    ...
    """Generate ``count`` single-answer puzzles into the session buffer.
    Allowed while ``preparing`` (initial buffer) and while ``active``
    (background refill). Returns the created puzzles (public fields only
    leave the API)."""
...
def issue_puzzle(
    ...
    session = _require_preparable(db, session_id)
    puzzle = generator.create_puzzle(db, rng, exactly_one=True)
...
def finish(db: Session, session_id: str) -> TrappedSpeedSession:
    session = get_session(db, session_id)
...
def summary(session: TrappedSpeedSession) -> dict[str, Any]:

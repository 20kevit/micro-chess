...
def test_guest_practice_creates_no_rating(client, db_session):
    # Guests are blocked before any attempt exists, so no rating state
...
def test_phase3_database_upgrades_to_v6_preserving_data():
    ...
    assert ensure_schema(engine) == SCHEMA_VERSION == 13
    assert ensure_schema(engine) == 13  # idempotent re-run

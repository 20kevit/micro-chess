# Exercise 8 — Pathfinding with Obstacles (مسیریابی با مانع)
...
This is the direct, more advanced evolution of **Exercise 7 —
...
Same formula as Exercise 7. Registered via
...
Same shell and lifecycle as Exercise 7 (`PathfindingObstaclesPlay`:
...
- Speed: `preparing` → `active` → `finished`/`expired`
  (`pathfinding_obstacles_speed_sessions`, same columns as Exercise 7),
...
strings: `exercises.pathfinding-obstacles.title/desc` («مسیریابی با
